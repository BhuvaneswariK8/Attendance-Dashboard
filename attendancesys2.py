...
import datetime
import matplotlib
matplotlib.use('Agg')  # Headless mode to prevent web UI freezing
...
def take_attendance_admin():
    ...
    class_name = st.selectbox(
        ...
        on_change=clear_widget_cache  # Clear checkbox cache on class change
    )
    ...
    with st.form(f"admin_attendance_form_{class_name}"):
        ...
        for index, row in df.iterrows():
            ...
            initial_status = (str(row[today_date]).strip().upper() == 'P')
            
            # Isolated key per class, roll number, and date
            is_present = st.checkbox(
                f"**{roll_no}** - {student_name}", 
                value=initial_status, 
                key=f"check_admin_{class_name}_{roll_no}_{today_date}"
            )
        ...
        if not st.session_state["teacher_authenticated"]:
            ...
            with st.form(f"teacher_attendance_form_{class_name}"):
                ...
                for index, row in df.iterrows():
                    ...
                    # Unique scoped key avoids cross-class memory overlap
                    checkbox_state = st.checkbox(
                        f"**{roll_no}** - {student_name}",
                        value=initial_status,
                        key=f"check_teacher_{class_name}_{roll_no}_{today_date}"
                    )
