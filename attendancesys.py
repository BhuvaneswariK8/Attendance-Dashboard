...
import datetime
import matplotlib
matplotlib.use('Agg')  # Prevents matplotlib GUI freezing in web apps
...
def take_attendance_admin():
    ...
    class_name = st.selectbox("Select Class", list(class_passwords.keys()), key="admin_take_class_select")
    ...
    with st.form(f"admin_attendance_form_{class_name}"):
        ...
        for index, row in df.iterrows():
            ...
            is_present = st.checkbox(
                ...
                key=f"admin_check_{class_name}_{index}"
            )
...
def main():
    ...
    if portal == "Class Teacher Portal":
        if not st.session_state["teacher_authenticated"]:
            ...
            if not os.path.exists(file_name):
                st.warning(f"File not found for {class_name}.")
                if st.button(f"Create Sample File for {class_name}", key=f"create_t_{class_name}"):
                    sample_df = pd.DataFrame({
                        "Roll_No": [101, 102, 103],
            ...
            with st.form(f"teacher_attendance_form_{class_name}"):
                ...
                for index, row in df.iterrows():
                    ...
                    checkbox_state = st.checkbox(
                        f"**{roll_no}** - {student_name}",
                        value=initial_status,
                        key=f"t_check_{class_name}_{index}"
                    )
